...
static int enet_poll(struct pico_device *dev, int loop_score)
{
    ...
    uint32_t ts;
    while(loop_score > 0) {
        if (ENET_GetRxFrameSize(&g_handle, &size,0) == kStatus_ENET_RxFrameEmpty)
            return loop_score;
        ENET_ReadFrame(enet->base, &g_handle, g_frame, size,0, &ts);
...
{
    struct pico_device_enet *enet = (struct pico_device_enet *)dev;
    if (ENET_SendFrame(enet->base, &g_handle, buf, len,0,1,NULL) != kStatus_ENET_TxFrameBusy)
        return len;
    return 0;
}
